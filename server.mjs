...
const types = { '.html': 'text/html; charset=utf-8', '.css': 'text/css', '.js': 'text/javascript', '.json': 'application/json', '.webmanifest':'application/manifest+json', '.svg': 'image/svg+xml', '.png': 'image/png', '.ico': 'image/x-icon' };
...
const server=http.createServer(async (req, res) => {
  try {
    ...
    if(!['/','/index.html','/publish.html','/favicon.svg','/manifest.webmanifest'].includes(pathname) && !/^\/(src|vendor)\/[A-Za-z0-9_.-]+\.(js|css)$/.test(pathname) && !/^\/assets\/[a-z-]+\.(json|png)$/.test(pathname)){res.writeHead(404).end('Not found');return;}
