...
(async()=>{
  ...
  try{
    ...
    await guest.waitForFunction(()=>window.__cookingdual.game.stations.find(s=>s.id==='counter-a').item?.kind==='meal');
    await host.waitForFunction(()=>window.__cookingdual.game.player.hand===null);assert.equal(counter.item.parts.length,2);
    await guest.locator('#action-touch').tap();await host.waitForFunction(()=>window.__cookingdual.game.players[1].hand?.kind==='meal');
    const plateStation=face(room.game,'counter-b',b);plateStation.item={kind:'plate',parts:[]};room.game.cleanPlates--;
    await guest.waitForFunction(()=>window.__cookingdual.game.target?.id==='counter-b');await guest.locator('#action-touch').tap();await host.waitForFunction(()=>window.__cookingdual.game.stations.find(s=>s.id==='counter-b').item?.kind==='plate'&&window.__cookingdual.game.stations.find(s=>s.id==='counter-b').item.parts.length===2);
    ...
    await guest.waitForFunction(()=>window.__cookingdual.game.served===1);assert.equal((await state(host)).score,(await state(guest)).score);pass('one chef assembles without a plate; the other picks it up, plates and serves with shared score');
    ...
    for(const p of [host,guest]){
      ...
      assert.equal(graphics.assets,13);assert.ok(graphics.angle>36&&graphics.angle<40);assert.equal(graphics.x,0);assert.equal(graphics.guide,'none');assert.equal(graphics.context,'none');assert.equal(graphics.orderText,'');assert.equal(graphics.images,true);assert.ok(graphics.labels.every(t=>!t));
    }
